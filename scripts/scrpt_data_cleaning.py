...
def main():
    ...
    write_test_file_path = os.path.join(os.path.dirname(__file__), "..", 'data', 'test_set_VU_DM_cleaned.csv')

    data_frame = read_csv(read_file_path)
    test_data_frame = read_csv(read_test_file_path)

    cleaned_data_frame = data_frame.copy()
    test_cleaned_data_frame = test_data_frame.copy()

    cleaned_data_frame = remove_duplicates(cleaned_data_frame)
    test_cleaned_data_frame = remove_duplicates(test_cleaned_data_frame)
    ...
    cleaned_test_data_frame = replace_iqr_outliers_with_na(test_cleaned_data_frame, 'prop_location_score1', iqr_thresh=1.5)
    cleaned_test_data_frame = replace_iqr_outliers_with_na(test_cleaned_data_frame, 'prop_location_score2', iqr_thresh=1.5)
    cleaned_test_data_frame = replace_iqr_outliers_with_na(test_cleaned_data_frame, 'prop_log_historical_price', iqr_thresh=1.5)
    cleaned_test_data_frame = replace_iqr_outliers_with_na(test_cleaned_data_frame, 'price_usd', iqr_thresh=1.5)
    cleaned_test_data_frame = replace_iqr_outliers_with_na(test_cleaned_data_frame, 'srch_length_of_stay', iqr_thresh=1.5)
    cleaned_test_data_frame = replace_iqr_outliers_with_na(test_cleaned_data_frame, 'srch_booking_window', iqr_thresh=1.5)
    ...
    analyze_data(cleaned_test_data_frame, name_extension='dataset_test_clean_')
    create_all_attribute_distributions(cleaned_test_data_frame, name_extension=os.path.join('test_clean', 'dataset_test_clean_'))

    dataframe_to_csv(cleaned_data_frame, write_file_path)
    dataframe_to_csv(cleaned_test_data_frame, write_test_file_path)
