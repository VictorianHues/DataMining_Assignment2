...
def load_data(aggregated_path: str, original_path: str):
    ...
    original_df['prop_review_score'] = original_df['prop_review_score'].fillna(0)
    original_df['price_usd'] = original_df['price_usd'].fillna(original_df['price_usd'].median())
...
def find_nearest_neighbors(model, model_df, target_id: int, k: int):
    if target_id not in model_df.index:
        raise ValueError(f"Target search ID {target_id} not found in aggregated data.")

    distances, indices = model.kneighbors(model_df.loc[[target_id]], n_neighbors=k + 1)
...
def recommend_hotels(original_df: pd.DataFrame, target_id: int, neighbor_ids: list):
    ...
    unseen_hotels = neighbor_hotels[~neighbor_hotels['prop_id'].isin(target_prop_ids)]
    unseen_hotels_unique = unseen_hotels.drop_duplicates(subset='prop_id').copy()

    # Ratio-based scoring with checks
    min_price = 10.0
    epsilon = 1e-6

    unseen_hotels_unique['prop_review_score'] = unseen_hotels_unique['prop_review_score'].clip(lower=0, upper=5)
    unseen_hotels_unique['adjusted_price'] = unseen_hotels_unique['price_usd'].clip(lower=min_price)
    unseen_hotels_unique['ratio_score'] = (
        unseen_hotels_unique['prop_review_score'] / (unseen_hotels_unique['adjusted_price'] + epsilon)
    )

    sorted_hotels = unseen_hotels_unique.sort_values(by='ratio_score', ascending=False)
    recommendations = sorted_hotels.head(len(target_hotels))
...
def generate_recommendations_for_all(aggregated_path, original_path, k_neighbors=5):
    ...
    for srch_id in tqdm(unique_srch_ids, desc="Recommending"):
        try:
            neighbor_ids, _ = find_nearest_neighbors(model, model_df, srch_id, k_neighbors)
            recommendations = recommend_hotels(original_df, srch_id, neighbor_ids)
